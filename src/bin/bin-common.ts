...
const commandLineParams: {[param: string]: Params} = {
  ...
  nodePath: {
    ...
    args: '<path>',
    description:
      'Path to database node (has to be a collection) where import will to start (e.g. collectionA/docB/collectionC). Imports at root level if missing.',
  },
  yesToImport: {
    shortKey: 'y',
    key: 'yes',
  ...
  yesToClear: {
    ...
    key: 'yes',
    description:
      'Unattended clear without confirmation (like hitting "y" from the command line).',
  },
  yesToNoWait: {
    shortKey: 'w',
    key: 'noWait',
    description:
      'Use with unattended confirmation to remove the 5 second delay.',
  },
  prettyPrint: {
    shortKey: 'p',
    key: 'prettyPrint',
...
const buildOption = ({
  ...
  args = '',
  description,
}: Params): [string, string] => [`-${shortKey} --${key} ${args}`, description];
