...
describe('Firestore Export', () => {
  ...
  it('should remove only child document', async () => {
    const initial = await mockFirestore
      .doc('collectionA/docA/contacts/contactDocId')
      .get();
    expect(initial.exists).to.equal(true);
    await firestoreClear(
      mockFirestore.doc('collectionA/docA/contacts/contactDocId')
    );
